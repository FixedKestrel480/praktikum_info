...
void PKW::vZeichnen(const Weg& weg)const {
	bZeichnePKW(
			getName(),
			weg.getName(),
			getAbschnittStrecke() / weg.getLaenge(),
			dGeschwindigkeit(), // Aktuelle Geschwindigkeit
		    getTankinhalt()// Kraftstoffstand
			);
}
...
void PKW::setTankinhalt(double dTank) {
	// Der Tankfüllstand darf die maximale Kapazität nicht überschreiten oder negativ sein
	if (dTank < 0.0) {
		p_dTankinhalt = 0.0;
	} else if (dTank > p_dTankvolumen) {
...
/**
 * Legt den Gesamtkraftstoffverbrauch fest.
 * dTank-Parameter Gesamtmenge des verbrauchten Kraftstoffs.
 */
void PKW::setGesamtVerbrauch(double dTank) {
	if (dTank >= 0.0) {
		p_dGesamtverbrauch = dTank;
	    } else {
	        std::cerr << "Fehler: Der Gesamtkraftstoffverbrauch kann nicht negativ sein.\n";
	    }
}


double PKW::dGeschwindigkeit() const {
	if (p_pVerhalten) { //Falls dem Auto ein Pfad zugeordnet ist
			const Weg& weg = p_pVerhalten->getWeg(); //Identifiziert den Pfad, auf dem es sich befindet
			double dTempolimit = weg.getTempolimit(); //Erreiche die maximale Geschwindigkeit
			// Gibt die durch Tempolimit begrenzte Geschwindigkeit zurück
			return std::min(Fahrzeug::dGeschwindigkeit(), dTempolimit);
		}
		return Fahrzeug::dGeschwindigkeit(); // Wenn kein zugehöriger Pfad vorhanden ist
}
